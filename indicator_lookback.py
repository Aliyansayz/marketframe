...
class  indicators_lookback_mode( access_indicators ):
  ...
  def transform_data_list(cls, refine_list ,  multiplier= 1.7 , atr_period = 5,  adx_period = 8,   lookback = 10, ema_period = [5, 20], ha_ohlc = True ):
    ...
    stochastic_oscillator_list =  indicator.stochastic_oscillator_lookback( bar_list = refine_list, period = 14, lookback = lookback)

    emaz_list  =  indicator.ema_lookback( bar_list = refine_list, ema_period = ema_period , lookback = lookback )
...
# ('heikin-ashi-ohlc', object),
    dt   = np.dtype([ ('index', 'datetime64[h]'),  ('symbol', 'U20'), ('Open', float ), ('High', float ), ('Low', float ),   ('Close', float ),  ('Heikin-Ashi-Status', 'U10'),  ('Direction', float), ('Average-Directional-Index', float), ('Crossover', float),   \
              ('Stop_Loss', float), ('Take_Profit', float),  ('ema20', float), ('ema5', float), ('bb_lower', float ), ('bb_upper', np.float64), ('atr_lower', float), ('atr_upper', float ), ('ha_open', float), ('ha_high', float),('ha_low', float),('ha_close', float)  ])
#
    ...
    for sym , ohlc in enumerate(refine_list[values]):
            ...
            k_percent = [ element[0] for element in stochastic_oscillator_list[sym] ]
            d_percent = [ element[1] for element in stochastic_oscillator_list[sym] ]
...
#               [ha_ohlc[0][i], ha_ohlc[1][i], ha_ohlc[2][i], ha_ohlc[3][i]],
                ...
                ohlc_df[i] = ( index[i], symbol[i], open[i], high[i], low[i], close[i], heikin_ashi_status[i], direction[i], adx_value[i], \
                               crossover[i], stop_loss, take_profit,  short_ema[i], long_ema[i],  bb_lower[i], bb_upper[i], lower_band[i], upper_band[i], ha_open[i], ha_high[i] , ha_low[i], ha_close[i]  )
#
